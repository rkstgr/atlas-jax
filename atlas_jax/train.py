...
- Time-budgeted training for autonomous experimentation
"""
...
def main():
    ...
    try:
        ...
        if result.returncode == 0:
            peak_vram_mb = max(float(x) for x in result.stdout.strip().split('\n'))
    except Exception:
        pass
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
            vram_now = max(float(x) for x in result.stdout.strip().split('\n'))
            peak_vram_mb = max(peak_vram_mb, vram_now)
    except Exception:
        pass

    # Compute average MFU from step times (excluding first few for warmup)
    ...
    # Structured summary (grep-friendly)
    print("---")
